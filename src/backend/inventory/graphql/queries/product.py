...
from inventory.graphql.services.crud import ProductCRUDService
from inventory.graphql.services.query import ProductQueryService
...
class Query(ObjectType):
    ...
    def resolve_product_by_id(self, info, product_id):
        try:
            ...
            return ProductCRUDService.get(id=product_id)
    ...
    def resolve_products_by_category(self, info, category_id):
        try:
            ...
            return ProductQueryService.get_by_category(category_id)
        except ValueError:
            raise GraphQLError("Invalid category ID")
    
    def resolve_low_stock_products(self, info, threshold):
        return ProductQueryService.get_low_stock_products(threshold)
